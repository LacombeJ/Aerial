...
public class Quaternion {
    ...
    public Quaternion setEulerAnglesDeg (float yaw, float pitch, float roll) {
        return setEulerAngles(yaw * Mathf.DEGREES_TO_RADIANS, pitch * Mathf.DEGREES_TO_RADIANS, roll
            * Mathf.DEGREES_TO_RADIANS);
    }
    ...
    public float getRollDeg () {
        return getRoll() * Mathf.RADIANS_TO_DEGREES;
    }
    ...
    public float getPitchDeg () {
        return getPitch() * Mathf.RADIANS_TO_DEGREES;
    }
    ...
    public float getYawDeg () {
        return getYaw() * Mathf.RADIANS_TO_DEGREES;
    }
...
// method
    ...
    /**
     * Sets the given matrix as a matrix representation of this quaternion
     * @param mat
     * @return the given matrix
     */
    public Matrix4 toMatrix (Matrix4 mat) {
        final float xx = x * x;
        final float xy = x * y;
        final float xz = x * z;
        final float xw = x * w;
     ...
     * @return This quaternion for chaining. */
    public Quaternion setFromAxis (final Vector3 axis, final float radians) {
    ...
    public Quaternion setFromAxisDeg (final float x, final float y, final float z, final float degrees) {
        return setFromAxis(x, y, z, degrees * Mathf.DEGREES_TO_RADIANS);
    }
     ...
     * @param radians The angle in radians
     * @return This quaternion for chaining. */
    public Quaternion setFromAxis (final float x, final float y, final float z, final float radians) {
        float d = Mathf.mag(x, y, z);
        if (d == 0f) return idt();
        d = 1f / d;
        float l_ang = radians < 0 ? Mathf.TWO_PI - (-radians % Mathf.TWO_PI) : radians % Mathf.TWO_PI;
    ...
    public Quaternion setFromMatrix (boolean normalizeAxes, Matrix4 matrix) {
        return setFromAxes(normalizeAxes, matrix.m00, matrix.m01, matrix.m02,
            matrix.m10, matrix.m11, matrix.m12, matrix.m20,
    ...
    public Quaternion setFromAxes (boolean normalizeAxes, float xx, float xy, float xz, float yx, float yy, float yz, float zx,
        float zy, float zz) {
        if (normalizeAxes) {
            final float lx = 1f / Mathf.mag(xx, xy, xz);
            final float ly = 1f / Mathf.mag(yx, yy, yz);
            final float lz = 1f / Mathf.mag(zx, zy, zz);
            xx *= lx;
            xy *= lx;
            xz *= lx;
            yx *= ly;
    ...
    public Quaternion setFromCross (final float x1, final float y1, final float z1, final float x2, final float y2, final float z2) {
        final float dot = Mathf.clamp(Mathf.dot(x1, y1, z1, x2, y2, z2), -1f, 1f);
        final float angle = (float)Math.acos(dot);
...
// value
    ...
    public float getAxisAngleDeg (Vector3 axis) {
        return getAxisAngle(axis) * Mathf.RADIANS_TO_DEGREES;
    }
    ...
    /** Get the angle in degrees of the rotation this quaternion represents. Use {@link #getAxisAngleDeg(Vector3)} to get both the axis
     * and the angle of this rotation. Use {@link #getAngleAroundDeg(Vector3)} to get the angle around a specific axis.
     * @return the angle in degrees of the rotation */
    public float getAngleDeg () {
        return getAngle() * Mathf.RADIANS_TO_DEGREES;
    }
    ...
    public void getSwingTwist (final float axisX, final float axisY, final float axisZ, final Quaternion swing,
        ...
        final float d = Mathf.dot(this.x, this.y, this.z, axisX, axisY, axisZ);
    ...
    public float getAngleAroundDeg (final float axisX, final float axisY, final float axisZ) {
        return getAngleAround(axisX, axisY, axisZ) * Mathf.RADIANS_TO_DEGREES;
    }
