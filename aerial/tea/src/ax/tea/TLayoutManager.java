...
import ax.commons.structs.ArrayList2D;
...
class TLayoutManager {
    ...
    static void allocateExtra(ArrayList<SizePreference> policy, Wrapper<Integer> extraDimension, int[] size) {
        ArrayList<SizePreference> rest = ax.commons.func.List.sort(policy, (a,b) -> Integer.compare(a.max, b.max));
        
        while (rest.size() > 0) {
            SizePreference p = ax.commons.func.List.first(rest);
            ...
            if (rest.size()>1) {
                rest = ax.commons.func.List.tail(rest);
            } else {
